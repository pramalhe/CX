...
int main(void) {
    ...
    vector<int> threadList = { 1, 2, 4, 8, 16, 32, 64}; // For Cervino
    ...
    for (unsigned iratio = 0; iratio < ratioList.size(); iratio++) {
        ...
        for (unsigned ithread = 0; ithread < threadList.size(); ithread++) {
            ...
            //results[iclass++][ithread][iratio] = bench.benchmark<UCSet<PSimOpt<TreeSet<UserData>>,TreeSet<UserData>,UserData>,UserData>               (cNames[iclass], ratio, testLength, numRuns, numElements, false,0);
            //results[iclass++][ithread][iratio] = bench.benchmark<UCSet<CXMutationWF<TreeSet<UserData>>,TreeSet<UserData>,UserData>,UserData>          (cNames[iclass], ratio, testLength, numRuns, numElements, false,0);
            ...
            //results[iclass++][ithread][iratio] = bench.benchmark<UCSetBlocking<CXMutationBlocking<TreeSet<UserData>>,TreeSet<UserData>,UserData>,UserData>          (cNames[iclass], ratio, testLength, numRuns, numElements, false, 32);
            // Natarajan's tree is just too slow (takes more than 2h to fill up the 1M keys): 10:07 -> 12:20
            //results[iclass++][ithread][iratio] = bench.benchmark<NatarajanTreeHE<UserData,UserData>,UserData>                                         (cNames[iclass], ratio, testLength, numRuns, numElements, false);
            maxClass = iclass;
        }
    }
