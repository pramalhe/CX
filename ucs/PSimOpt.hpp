 ...
 */
#ifndef _PSIM_OPTIMIZED_UNIVERSAL_WF_H_
#define _PSIM_OPTIMIZED_UNIVERSAL_WF_H_
...
/**
 * <h1> Optimized P-Sim universal wait-free construct </h1>
 *
 ...
 * - The apply_op() method is named applyMutation() and there is a new method
 *   named applyRead() which can be used for read-only operations, thus giving
 *   much better throughput. The applyRead() method defaults to applyMutation()
 *   if it fails to acquire the hazard pointer on the data structure instance
 *   for more than 10 attempts;
...
public:
    ...
    PSimOpt(C* inst, const int maxThreads=MAX_THREADS) : maxThreads{maxThreads} {
        SeqPointer first;
    ...
    static std::string className() { return "PSimOpt-"; }
     ...
     */
    R applyRead(std::function<R(C*)>& readOnlyFunc, const int tid) {
        for (int i = 0; i < MAX_READ_TRIES; i++) {
            SeqPointer lptr = objPointer.load();
            C* inst = hpInst.protectPtr(kHpInst, objStates[lptr.u.index].instance.load(), tid);
            if (lptr.raw != objPointer.load().raw) continue;
            // "inst" is now protected by an HP
            R retVal = readOnlyFunc(inst);
            hpInst.clear(tid);
            return retVal;
        }
        // We can't get the hp on inst, so we have to go with a mutative operation instead
        return applyUpdate(readOnlyFunc, tid);
    }
};

#endif /* _PSIM_OPTIMIZED_UNIVERSAL_WF_H_ */
