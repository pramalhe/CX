...
// This isn't working... maybe because of copyPtr() should swap instead of copying

#ifndef _NATARAJAN_TREE_HAZARD_POINTERS_H_
#define _NATARAJAN_TREE_HAZARD_POINTERS_H_
...
private:
    ...
    /* variables */
    HazardPointers<Node> hp {5, MAX_THREADS};
...
public:
    NatarajanTreeHP(const int maxThreads=0) {
    ...
    std::string className() { return "NatarajanTreeHP"; }
...
//-------Definition----------
template <class K, class V>
void NatarajanTreeHP<K,V>::seek(K key, int tid){
    ...
    seekRecord->successor = hp.get_protected(1, r->left, tid);
    seekRecord->parent = hp.get_protected(2, r->left, tid);
    seekRecord->leaf = getPtr(hp.get_protected(3, s->left, tid));

    /* initialize other variables used in the traversal */
    Node* parentField = hp.get_protected(3, seekRecord->parent->left, tid);
    Node* currentField = hp.get_protected(4, seekRecord->leaf->left,tid);
    ...
    while(current!=nullptr){
        ...
        if(nodeLess(&keyNode,current)){
            currentField = hp.get_protected(4, current->left, tid);
        }
        else{
            currentField = hp.get_protected(4, current->right, tid);
        }
...
template <class K, class V>
bool NatarajanTreeHP<K,V>::cleanup(K key, int tid){
    Node keyNode{key,defltV,nullptr,nullptr};//node to be compared
    bool res=false;
...
template <class K, class V>
std::optional<V> NatarajanTreeHP<K,V>::get(K key, int tid){
    Node keyNode{key,defltV,nullptr,nullptr};//node to be compared
    std::optional<V> res={};
    SeekRecord* seekRecord=&(records[tid]);
    Node* leaf=nullptr;
...
template <class K, class V>
std::optional<V> NatarajanTreeHP<K,V>::put(K key, V val, int tid){
    std::optional<V> res={};
    SeekRecord* seekRecord=&(records[tid]);
...
template <class K, class V>
bool NatarajanTreeHP<K,V>::insert(K key, V val, int tid) {
    bool res=false;
    SeekRecord* seekRecord=&(records[tid]);
...
template <class K, class V>
std::optional<V> NatarajanTreeHP<K,V>::innerRemove(K key, int tid){
    bool injecting = true;
    std::optional<V> res={};
    SeekRecord* seekRecord=&(records[tid]);
...
template <class K, class V>
std::optional<V> NatarajanTreeHP<K,V>::replace(K key, V val, int tid){
    std::optional<V> res={};
    SeekRecord* seekRecord=&(records[tid]);
...
template <class K, class V>
std::map<K, V> NatarajanTreeHP<K,V>::rangeQuery(K key1, K key2, int& len, int tid){
    //NOT HP-like GC safe.
    if(key1>key2) return {};
    Node k1{key1,defltV,nullptr,nullptr};//node to be compared
    Node k2{key2,defltV,nullptr,nullptr};//node to be compared

    Node* leaf = getPtr(hp.get_protected(0, s->left, tid));
    Node* current = getPtr(hp.get_protected(1, leaf->left, tid));
...
template <class K, class V>
void NatarajanTreeHP<K,V>::doRangeQuery(Node& k1, Node& k2, int tid, Node* root, std::map<K,V>& res){
    Node* left = getPtr(hp.get_protected(2, root->left, tid));
    Node* right = getPtr(hp.get_protected(3, root->right, tid));
...
template <class K, class V>
bool NatarajanTreeHP<K,V>::add(K key, int tid) {
    return insert(key,key,tid);
}

template <class K, class V>
bool NatarajanTreeHP<K,V>::remove(K key, int tid) {
    return innerRemove(key,tid).has_value();
}

template <class K, class V>
bool NatarajanTreeHP<K,V>::contains(K key, int tid) {
    return get(key,tid).has_value();
}

// Not lock-free
template <class K, class V>
void NatarajanTreeHP<K,V>::addAll(K** keys, const int size, const int tid) {
